...
class Test(unittest.TestCase):
    ...
    def test_serializer(self):
        ...
        self.assertEqual(s.dumps(['1234'],95,context),'[1, ["1234"], 95]')
        self.assertEqual(s.dumps({'hello': 'world'},100,context),'[1, {"hello": "world"}, 100]')
        rec = {
                    'hello': 'world',
                    'key': { 'key2': 123 }
                  }
        complex_dump = s.dumps(rec,62,context)
        self.assertEqual(complex_dump,'[1, {"hello": "world", "key": {"key2": 123}}, 62]')
        ( complex_rec, message_id )  = s.loads(complex_dump,context)
        self.assertEqual(complex_rec,rec)
        self.assertEqual(message_id,62)

        # Now let's see it handle objects
        tc = TestClass()
        json_obj_dump = s.dumps(tc,45,context)
        ( dump_result, message_id ) = s.loads(json_obj_dump,context)
        ...
        json_req_dump = s.dumps(omi_req,54,context)
        self.assertEqual(json_req_dump,'[0, "object", "method", [1, ["a"]], [1, {"b": "c"}], 54]')
        ( dump_result, message_id ) = s.loads(json_req_dump,context)
        ...
        ## System Events
        se = SystemEvent("BOOM!")
        json_ev_dump = s.dumps(se,55,context)
        self.assertEqual(json_ev_dump,'[11, [1, "BOOM!"], 55]')
        ( dump_result, message_id ) = s.loads(json_ev_dump,context)
        ...
        json_ev_dump = s.dumps(se,56,context)
